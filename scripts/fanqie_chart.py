# -*- coding: utf-8 -*-
"""番茄音乐「热门歌曲」采集（网页版首页，**只读**）—— ⚠️ 已降级为**兜底**，首选 fanqie_rank.py

首选方案
────────
研究番茄音乐榜/热歌榜，请用 **`fanqie_rank.py`**（2026-09-21 打通）：
直连番茄音乐 APP 官方接口，拿到**平台自己的榜单**（与用户 APP 里看到的一致）。

本脚本（网页版首页）只在 APP 接口不可用时兜底，因为**网页版与 APP 差距很大**。
...
> ⚠️ **要 APP 官方榜请用 `fanqie_rank.py`，不要用本脚本。**
> 2026-09-21 已打通**番茄音乐 APP 官方榜单接口**（`aid=8661`，只读、无需签名/登录）：
> `python fanqie_rank.py --rank 热歌榜` 可拿到热歌/飙升/经典/70-00后/车载/运动等
> **10 个官方榜**、每榜 50 首，带**平台自己的名次 + 涨跌幅 + 真实播放量 + 官方曲风标签**。
> 那是用户在 APP 里看到的同一份数据。
>
> 本脚本（网页版）只作为**兜底**：网页版**没有榜单页**，且与 APP 榜差距很大。
> 原来的判断「APP 榜需手机抓包/模拟器」**已作废** —— 不需要抓包，直连接口即可。
