# -*- coding: utf-8 -*-
"""
⚠️⚠️ 历史备选端 · 已停用（2026-09）⚠️⚠️

    生成端已从 MiniMax 换成【妙响（抖音音乐创作实验室）】，
    当前主力脚本是 `miaoxiang.py`，**新流程请一律走它**。

    本文件保留的原因：妙响临时不可用时，可回退到 MiniMax 网页端救急。
    它已不再维护，selectors 漂移时不会有人来修——能不用就不用。

    妙响与 MiniMax 的关键差异：妙响 1 份歌词 → 固定出 2 个版本（数量不可改），
    所以本文件里的 `quantity_per_round` 数量控件逻辑对妙响**无效**。
