...
**★★ 代创作前必须先研究榜单，而且研究的是「怎么写」不是「多少数据」**（不要凭感觉写）：

> **用户的原话**：*「不是要你收集数据研究，是要研究歌曲的创作思维模式和创作风格，
> 数据没什么好研究的。」* —— 在听人数只用来**挑出研究哪几首**，不是研究目标本身。

| 顺序 | 动作 | 命令 / 位置 |
|---|---|---|
| **1（必做）** | **采番茄音乐 APP 官方榜 → 只为选样本** | `<python> <skill>/scripts/fanqie_rank.py --rank 热歌榜` |
| **2（★ 核心）** | **把 TOP N 的歌词拉下来拆解**（结构/记忆点/句式/韵脚/意象五层） | `<python> <skill>/scripts/fanqie_lyric.py --top 10` |
| 3 | 读历史积累（含**撞名黑名单**） | `references/HOT_SONG_RESEARCH.md` **最后一期** |
| 4 | 交叉第三方榜看大盘风向（抖音热歌榜 / 酷狗 / 短视频热歌榜） | 搜索；**与第 1 步冲突时以番茄自有数据为准** |
| 5 | 对比上一期看趋势，把本轮结论**追加写回** `HOT_SONG_RESEARCH.md` | 必须回答「听众情绪需求四问」 |
| 6 | 然后才动笔 | 按 `references/CREATION_GUIDE.md` 写 |

> **为什么第 2 步才是核心**：榜单只告诉你「哪首火」，**拆解才告诉你「它凭什么火」**
> —— 而这个「凭什么」才是能复制到我们自己歌里的东西。
> 第 1 步的产物在 `<工作目录>/charts/`，第 2 步的产物在 `<工作目录>/study/`（含每首歌词全文）。

> **★ 第 1 步为什么必须是番茄自有榜**：用户的听众在番茄，锚就必须打在番茄身上。
> 实测翻车过 —— 第十轮我全程用搜狐/酷狗/歌曲吧的第三方榜，得出「国风降为次要优先级」；
> 第十一轮采番茄自有榜后**结论直接反转**：国风在番茄是**第一大题材**
> （全样本 33/199、TOP30 6/30），《琵琶曲》一首占了 #2 和 #4 两个版本。
> **第三方榜反映全网上班族审美，番茄是下沉+中老年 —— 锚打错地方，结论就会反过来。**
...
| 你帮我写 | **先跑 `fanqie_rank.py --rank 热歌榜` 采番茄 APP 官方榜**（+ 读 `HOT_SONG_RESEARCH.md` + 第三方榜交叉，并追加新一期研究），再按 `CREATION_GUIDE.md` 创作，**成稿给用户过目确认后再生成** |
