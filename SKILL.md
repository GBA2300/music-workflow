...
### 顺手清掉旧工作目录（用户说「C 盘还占着，都删了吧」时走这条）

旧工作目录里的 `profile*`（历史登录态）动辄几百 MB，是**纯残值** ——
当前登录态在 `%LOCALAPPDATA%/music-workflow/profiles/`（由 `paths.user_profile()` 解析），
**不在工作目录里**，所以删旧目录不会让用户重新登录。
⚠️ 反过来：`%LOCALAPPDATA%/music-workflow/profiles/` **绝对不能删**。

**执行顺序（缺一步就会「删了但空间没少」）：**

```
1. 拆分类别 → 看清有多少是 profile*/截图/缓存，多少是有价值的记录
2. 备份有价值的小东西（published.json / tasks.csv / lyrics / config.json / 封面底图）
   → <workdir>/_旧目录存档/<日期>__<目录名>/
3. 交叉校验 + MD5：确认要删的没有「目标侧不存在的孤儿」、没有独一份内容
4. 删目录            ← ⚠️ 必须【前台】跑，见下
5. 清回收站          python purge_bin.py --from-report "<workdir>" --yes
6. 磁盘实测确认释放量
```

**⚠️ 两个必知的机制（2026-09-20 实测踩透）：**

1. **主机会把 Python 的删除改道到回收站。** `PYTHONPATH` 注入的 `sitecustomize.py`
   拦截 `os.remove` / `os.rmdir` / `shutil.rmtree`，所以脚本里写 `shutil.rmtree(d)`
   **只是把 d 挪进回收站** —— 回收站同盘 → 空间一点没少（实测释放 **-9.6 MB**）。
   **删完必须再清回收站**，否则用户会觉得「你根本没删」。
2. **批量删除守卫（阈值 50 个文件）只在【前台】运行时才弹授权提示。**
   后台跑（`run_in_background`）会在 2 秒内直接失败并打印
   `[safe-delete][SAFE_DELETE_BULK_CONFIRM_REQUIRED]`，**看起来像命令写错了**，
   其实只是没人能点「允许」。→ **改前台重跑**就过。

- 单独清任意目录的回收站条目（不限于迁移报告）：
  `<python> <skill>/scripts/purge_bin.py --path "C:\...\old-dir" --yes`
  （不加 `--yes` 就是 dry-run）
- ⚠️ 删一棵树在回收站里是 **1 + 树内文件数** 条（不是 1 条）——
  实测 8 个目录 → **459 条**。清的时候匹配条件必须覆盖「目录本身」和「目录之下」两种
