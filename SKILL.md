...
| **你帮我写** | AI 代创作 | **先走完「纪律 0」六步铁律**（研究热歌榜前十首 → 出创作配方卡 → 答情绪需求四问），再按 `references/CREATION_GUIDE.md` 创作，写完给用户过目 |
| **混合** | 用户给主题/方向，或用户写词、AI 优化风格描述 | 先确认分工边界，再动笔 |

**★★★ 铁律：每次代创作前，必须先研究番茄音乐 APP 热歌榜的前十首，学它的创作思维模式与创作风格**

> **用户的原话（两次强调，都要遵守）**：
> ① *「不是要你收集数据研究，是要研究歌曲的**创作思维模式和创作风格**，数据没什么好研究的。」*
> ② *「记得你在创作歌曲之前要研究学习番茄音乐听歌平台**音乐榜或者热歌榜的前十首**音乐，
> 收集灵感并学习歌曲的创作思维模式和创作风格，创作**符合番茄音乐平台受众人群**的爆款音乐歌曲，
> 能**挖掘洞察听众用户的情绪需求**，产生情感共鸣。」*
>
> **播放量/占比只是用来「挑研究哪几首」的，不是研究目标本身。** 研究目标永远是：
> **这些歌怎么写 + 听众要什么情绪 → 我这首该怎么写。**

**六步（一步都不许跳；第 1~4 步是「研究 + 落笔前」，第 6 步才动笔）**：

| 顺序 | 动作 | 命令 / 位置 |
|---|---|---|
| **1（必做）** | **采番茄音乐 APP 官方热歌榜**（拿官方名次/涨跌/播放量/官方曲风标签，只为选样本） | `<python> <skill>/scripts/fanqie_rank.py --rank 热歌榜 --limit 50` |
| **2（★ 核心）** | **把前十首的歌词拉下来，拆五层**（结构/记忆点/句式/韵脚/意象） | `<python> <skill>/scripts/fanqie_lyric.py --top 10 --rank 热歌榜` |
| **3（★ 落地）** | **生成创作配方卡**（把「榜单 + 拆解」压成能照着落笔的数字） | `<python> <skill>/scripts/fanqie_recipe.py --ranks 热歌榜,80后热歌,90后热歌` |
| **4（★ 情绪）** | **回答「听众情绪需求四问」** —— 四问不答完不许开写 | 打开 `study/番茄创作配方-<日期>.md` **第七节**，逐条答 |
| 5 | 读历史积累（含**撞名黑名单**）+ 交叉第三方榜看大盘风向 | `references/HOT_SONG_RESEARCH.md` **最后一期**；冲突时**以番茄 APP 官方榜为准** |
| **6** | 对比上一期看趋势，把本轮结论**追加写回** `HOT_SONG_RESEARCH.md`，**然后才动笔** | 按 `references/CREATION_GUIDE.md` 写 |

> **各步产物**：第 1 步 → `<工作目录>/charts/番茄榜单-<榜名>-<日期>.md/.json`
> ｜第 2 步 → `<工作目录>/study/番茄歌词拆解-<日期>.md/.json`（含每首歌词全文）
> ｜第 3 步 → `<工作目录>/study/番茄创作配方-<日期>.md/.json`（**配方卡**）
>
> **为什么第 2、3 步才是核心**：榜单只告诉你「哪首火」，**拆解才告诉你「它凭什么火」**，
> **配方才告诉你「你这首该怎么写」** —— 拆解出的是**指标**（行数 / 重复几次 / 韵脚），
> 配方把指标变成**参数**（写几行、副歌几行、整块重复几次、歌名几个字）。
> **指标不能直接下笔，参数才能。**

> **★ 第 1 步为什么必须是番茄自己的榜**：用户的听众在番茄，锚就必须打在番茄身上。
> **数据源选错不是「不够精确」，是「结论相反」** —— 实测翻车了两次，教训是叠加的：
> 1. 用**第三方榜**（搜狐/酷狗/歌曲吧）→ 得出「国风是**次要**题材」；
> 2. 换**番茄网页版**数据 → 结论**反转**成「国风是**第一大**题材」；
> 3. 再换**番茄 APP 官方榜（现行口径）** → 又修正为「**流行/情歌/伤感/轻柔/DJ 才是主盘，
>    国风不在标签前列**」。
>
> **两条教训**：① **锚要打在番茄身上**（别用第三方榜）；
> ② **口径要一致**（网页版人工归类 ≠ APP 官方标签）——
> **换数据源后旧结论必须重估，绝不可直接沿用。**

> ✅ **2026-09-21：番茄音乐 APP 官方榜已打通（首选做法，不用抓包）**
> `fanqie_rank.py` 直连番茄音乐 APP 自己的接口（`aid=8661` / `FmMusic`），
> **只读、无需签名、无需登录、无需 Cookie**，拿到的是**用户在 APP「排行榜」里看到的同一份数据**：
> - **10 个官方榜**：热歌榜 / 抖音榜 / 飙升榜 / 经典榜 / 70后热歌 / 80后热歌 / 90后热歌 / 00后热歌 / 车载热歌 / 运动热歌
> - 每榜 **50 首**，字段含**平台自己的名次**（列表顺序）、**涨跌幅**（▲N/▼N/—/新上榜）、
>   真实播放量、收藏数、订阅数、**官方曲风标签**、时长
> - 榜单曲目带的 `book_id` **可直接喂给 `fanqie_lyric.py`** → 榜单到拆解全链路打通
>
> 命令：`fanqie_rank.py --list-ranks` 看全部榜；`--rank 热歌榜` 单榜；`--rank all` 全采。
>
> ⚠️ **网页版 ≠ APP，差距很大**：网页版**没有榜单页**（18 个候选路径全回落首页、无榜单入口、
> 无列表接口），只有首页「热门歌曲」199 条。**不要再拿网页版当榜单交付。**
> `fanqie_chart.py`（网页版）**已降级为兜底**，仅在 APP 接口不可用时才用。
> **「APP 榜需手机抓包（Appium/mitmproxy）」这个旧判断已作废** —— 直连接口即可。
>
> **但这不影响创作研究** —— 我们要的**歌词与写法在歌曲详情页就能拿到**（第 2 步）。
> 另：详情页**不含**作词作曲署名，所以「谁写的」拿不到；
> 曲风标签改用 `fanqie_rank.py` 榜单自带的官方 `tags` 字段（比推断准）。

> 这么做的原因：凭印象写歌 = 碰运气。2026-08-30 首期研究就**推翻了原先两个判断**
> ——国风其实占比极高（原以为低优先级）、伤感情歌不必回避（原以为要避免）。
> **不研究就会沿着错误经验一路写下去。**

> **可以主动提改进（用户明确欢迎）**：原话 *「你也可以提供更好的创意和想法去改进歌曲创作，
> 最好能持续输出有爆款潜质的音乐」*。
> 若你在研究里发现能提高命中率的新规律（新的歌名形态、新的题材交叉、新的结构参数），
> **直接写进配方卡与 `HOT_SONG_RESEARCH.md`，并大胆用在下一首里** ——
> 但**必须留痕**（改了什么、依据是哪一轮哪一首）。
> 自检口径：**「这个改动，是数据看出来的，还是我想当然的？」** —— 后者不许写进配方。
...
│   ├── fanqie_rank.py     ★★ 榜单采集（纪律 0 第一步的**首选**）：番茄音乐 **APP 官方榜**
│   │                      直连 APP 接口（aid=8661），10 个官方榜 × 每榜 50 首，
│   │                      含平台名次/涨跌幅/播放量/收藏/官方曲风标签
│   │                      （只读、无需签名/登录/抓包。含 --list-ranks / --rank / --limit）
│   ├── fanqie_chart.py    ⚠️ 榜单采集（**兜底**，已降级）：番茄音乐**网页版**首页「热门歌曲」
│   │                      + 真实「在听人数」→ 按在听人数排 TOP N → 写 <workdir>/charts/
│   │                      （只读。网页版**没有**热歌榜页面且与 APP 差距大 → 首选 fanqie_rank.py）
│   │                      （用户给的是网页版数据时用它，否则别用）
│   ├── fanqie_lyric.py    ★★ 歌词拆解（纪律 0 第二步）：取平台在播版本歌词全文，
│   │                      拆成 结构/记忆点/句式/韵脚/意象 五层 → <workdir>/study/
│   │                      （只读。含 --top / --ids / --rank / --chart-json / --workdir）
│   │                      ⚠️ 样本默认取 APP 官方**热歌榜**前十首（--rank 可换榜）
│   ├── fanqie_recipe.py   ★★★ 创作配方卡（纪律 0 第三步）：把「榜单 + 拆解」压成
│   │                      可落笔的参数 —— 谁在听 / 写几行 / 副歌几行 / 重复几次 /
│   │                      歌名几个字 / 韵脚挑哪几个，并附 情绪需求四问 + 撞名清单 + 自检清单
│   │                      → <workdir>/study/番茄创作配方-<日期>.md/.json
│   │                      （纯本地计算，不联网。含 --ranks / --no-save / --workdir）
...
| 你帮我写 | **先走完「纪律 0」的六步**（`fanqie_rank.py --rank 热歌榜 --limit 50` → `fanqie_lyric.py --top 10 --rank 热歌榜` → `fanqie_recipe.py` → 答情绪需求四问 → 读 `HOT_SONG_RESEARCH.md` + 第三方榜交叉 → 追加写回研究档案），再按 `CREATION_GUIDE.md` 创作，**成稿给用户过目确认后再生成** |
