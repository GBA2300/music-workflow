...
**它按这个顺序处理**：
1. 点 × 关闭按钮（antd 的 `.ant-modal-close`、`aria-label='关闭'` 等 32 种常见写法）
2. 点不到就点文字按钮（「我知道了」「以后再说」等）
3. 再点不到就按 Esc
4. 还不行就**铲掉挡路的整屏遮罩**，再回到第 1 步重新点

第 4 步是关键：那种「整屏半透明遮罩、还没有关闭按钮」的浮层，会把关闭按钮自己也挡住，
不先铲掉遮罩就永远点不到。判断依据是遮罩内部**没有任何可交互元素**——真正的对话框
（里面有按钮/输入框）会被跳过，不会误删。

**什么时候需要人工介入**：
- 遇到一种新弹窗关不掉 → 把它的选择器加进 `config.json` 的 `popup_guard.close_selectors` **最前面**，
  不用改代码。想彻底关掉文字按钮自动点击，把 `allow_text_buttons` 设为 `false`。
- 想确认守卫是否正常工作 → 跑 `python test_popup_guard.py`（真起 chromium 真点一遍，14 项自检）。
