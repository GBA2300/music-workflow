...
| 顺序 | 动作 | 命令 / 位置 |
|---|---|---|
| **1（必做）** | **采番茄自有榜单 → 只为选样本** | `<python> <skill>/scripts/fanqie_chart.py --top 10` |
...
> ⚠️ **番茄音乐网页版没有「热歌榜」页面**（2026-09-21 实测，别再重复试）：
> 18 个候选路径（`/rank` `/chart` `/hot` `/toplist` `/music/rank` `/billboard` `/ranking` …）
> **全部回落首页**（SPA catch-all）；页面含「榜/排行/热歌」的可点元素 **0 个**；
> 也没有列表 JSON 接口（首页是 SSR 直出，网络里只有字节埋点 `*.zijieapi.com`）。
> **APP 端真正的榜单需手机抓包（Appium / mitmproxy），当前工具链不覆盖。**
> **但这不影响创作研究** —— 我们要的**歌词与写法在歌曲详情页就能拿到**（第 2 步）。
> 另：详情页**不含**作词作曲署名、**不含**曲风标签，所以「谁写的 / 什么曲风」拿不到，需另判。
...
│   ├── fanqie_chart.py    ★ 榜单采集（纪律 0 的第一步）：番茄音乐网页版首页「热门歌曲」
│   │                      + 真实「在听人数」→ 按在听人数排 TOP N → 写 <workdir>/charts/
│   │                      （只读。⚠️ 网页版**没有**热歌榜页面，APP 榜单需抓包，见类注释）
...
| 你帮我写 | **先跑 `fanqie_chart.py` 采番茄自有榜单**（+ 读 `HOT_SONG_RESEARCH.md` + 第三方榜交叉，并追加新一期研究），再按 `CREATION_GUIDE.md` 创作，**成稿给用户过目确认后再生成** |
