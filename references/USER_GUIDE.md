...
**结论：把工具拷给任何人，他自己第一次运行时打开浏览器登录自己的账号即可，绝不会用到你的账号。**
...
| 2 | 首次登录 MiniMax | 一次性 | 扫码/账号密码。登录态存本地 `profile/`，**之后长期自动免登**（Cookie 一般撑数周~数月，过期重跑 `--login` 即可） |
| 3 | 首次登录番茄 | 一次性 | 同上，登录态存 `profile_fanqie/` |
