...
>
> 🛡️ **还会自动做一道「孤儿交叉校验」**：逐条检查回收站里的东西在 D 盘是不是都有，
> 凡是 D 盘**没有**的（= 早就不在曲库里的历史残留），会先捞到 `D:\music-workflow\_recovered\`
> 保存好，再执行永久删除。实测救回过一张封面源图。
> **所以 `_recovered\` 这个目录不要删** —— 它是保险柜，不是垃圾。
>
> 📏 **怎么确认真腾出空间了**：比较清理前后 C 盘的「已用」数值（实测 204.65 GB → 204.31 GB，
> 释放 348 MB）。**别只看脚本打印的 MB 数** —— 它和实际会有出入。
> 另外「回收站条目数」会**远多于歌曲数**（45 首 → 266 条），这是正常的：
> 回收站是按**被删对象逐条**记的，每个文件一条，目录也各占一条。
